    ...
    <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-20 bg-primary`}>
      <div className="w-full flex justify-between items-center max-w-7xl mx-auto">
        <Link to="/" className="flex item-center gap-2" onClick={() => {
          ...
          <p className="text-white text-[18px] font-bold cursor-pointer flex">Dinesh &nbsp; <span className="sm:block hidden">| Frontend Engineer</span></p>
        </Link>
        <ul className="list-non hidden sm:flex flex-row gap-10">
          {navLinks.map((link) => (
            ...
            </li> // Assuming 'link' has a property 'name'
          ))}
        </ul>
